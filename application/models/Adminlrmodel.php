...
class Adminlrmodel extends CI_Model {
		...
		$this->form_validation->set_rules("password_confirmation", "Password Confirmation", "trim|required|matches[password]");
			if($this->form_validation->run() === FALSE) {
				...
				$query = "INSERT INTO users (username, name, email, password, created_at, updated_at) 
						  VALUES (?, ?, ?, ?, NOW(), NOW())";
				$values = array($post['username'], $post['name'], $post['email'], $post['password']);
	...
	public function login($post) {
		$query = "SELECT name, username, id FROM users WHERE username = ? AND password = ?";
